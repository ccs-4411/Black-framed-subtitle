...
def process_video_task(video, srt, resolution, pad_height, pos_y, font_zh, font_en, zh_size, en_size, color_zh, color_en, zh_bold, zh_italic, en_bold, en_italic, mode="full"):
    ...
    if not srt_input_path or not os.path.exists(srt_input_path):
        return None, "❌ 錯誤：硬碟中找不到實體字幕檔案，請重新上傳。"
    ...
    # 將 -i 放在前面，-ss 和 -t 放在後面（更相容）
    cmd += ["-i", working_video_path]
    if is_preview:
        # 限制只處理前 10 秒（放在 -i 之後，雖然速度稍慢但較可靠）
        cmd += ["-ss", "00:00:00", "-t", "10"]
    ...
    try:
        # 預覽超時延長至 60 秒，避免雲端環境資源不足
        timeout_seconds = 60 if is_preview else 300
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout_seconds)

        # 無論成功與否，都輸出詳細資訊到日誌
        log_detail = []
        if result.stdout:
            log_detail.append("【FFmpeg stdout】\n" + result.stdout)
        if result.stderr:
            log_detail.append("【FFmpeg stderr】\n" + result.stderr)

        if result.returncode == 0 and os.path.exists(output_path):
            if is_preview:
                return output_path, "🔄 10 秒動態效果預覽生成成功！\n" + "\n".join(log_detail)
            else:
                return output_path, "✨ 全片轉檔成功！\n" + "\n".join(log_detail)
        else:
            error_msg = f"❌ FFmpeg 執行失敗 (傳回碼 {result.returncode})。\n\n"
            error_msg += "\n".join(log_detail) if log_detail else "無額外日誌輸出，請檢查 FFmpeg 指令和輸入檔案。"
            return None, error_msg

    except FileNotFoundError:
        return None, "❌ 系統錯誤：找不到 FFmpeg 執行檔！請確認 Dockerfile 是否確實安裝 ffmpeg。"
    except subprocess.TimeoutExpired:
        return None, f"❌ 系統錯誤：FFmpeg 執行超時（{timeout_seconds}秒），請嘗試降低解析度（例如 854x480）以減輕雲端運算負擔。"
    except Exception as e:
...
with gr.Blocks(title="Python Video Toolbox V9.6", fill_height=True) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            gr.Markdown("### 🖥️ 【即時狀態與成果預覽】")
            log_output = gr.Textbox(label="狀態與即時日誌監控", placeholder="等待任務啟動...", lines=3, elem_classes="log-box")
