...
# =====================================================================
# 🛠️ 環境修復：自動偵測並強制修復 FFmpeg 在雲端環境上的執行路徑
# =====================================================================
...
# =====================================================================
# 📝 字幕高級解析：根據 UI 參數動態產生完全分離的中英文 ASS 樣式
# =====================================================================
def srt_to_ass(srt_path, ass_path, zh_font, zh_bold, zh_italic, zh_size, zh_color, en_font, en_bold, en_italic, en_size, en_color, v_offset):
    ...
    # 格式化顏色 (Gradio 的 #RRGGBB 轉為 ASS 的 &H00BBGGRR&)
    ...
    # 垂直位移：ASS 的 MarginV 基本值是 15，加上使用者的垂直位移調整
...
def process_video_task(
    ...
    progress(0, desc=f"🚀 正在配置 [{mode_text}] 進階參數環境...")
    ...
    # 1. 根據 UI 參數轉換字幕
    progress(0.2, desc="📝 正在動態編譯自訂中英雙語 ASS 字幕樣式...")
    ...
    log_msg = f"⚙️ UI 核心參數同步成功！\n📺 解析度: {resolution} | 黑框增高: {pad_height}px | 字幕垂直位移: {v_offset}px\n🎬 執行指令: {' '.join(cmd)}\n"
    print(log_msg)
    ...
    try:
        ...
        if os.path.exists(output_video) and os.path.getsize(output_video) > 0:
            progress(1.0, desc=f"✅ {mode_text} 成功！")
            return output_video, log_msg + f"\n🎉 處理完成！請在播放器中檢視成果。"
        else:
...
# =====================================================================
# 🎨 介面佈局：Gradio 100% 參數群組精確復刻
# =====================================================================
with gr.Blocks(theme=gr.themes.Soft(primary_hue="teal", secondary_hue="slate")) as demo:
    gr.Markdown("# 🎬 Python Video Toolbox v11.0 (旗艦參數完全版)")
    
    with gr.Row():
        # 左側控制面板
        with gr.Column(scale=1):
            gr.Markdown("### 📂 1. 來源檔案來源")
            ...
            with gr.Group():
                gr.Markdown("### 🔤 【中文字型樣式】")
                zh_font_dropdown = gr.Dropdown(choices=["WenQuanYi Zen Hei", "Arial", "Noto Sans CJK TC", "Sans-Serif"], value="WenQuanYi Zen Hei", label="中文字型")
                with gr.Row():
            ...
            with gr.Group():
                ...
                with gr.Row():
                    ...
                    en_color_input = gr.ColorPicker(label="字體顏色", value="#FFFF00") # 預設圖片中的黃色
            
            gr.Markdown("### ⚙️ 4. 工作流程")
            with gr.Row():
        ...
        # 右側輸出面板
        with gr.Column(scale=1):
            gr.Markdown("### 📺 5. 渲染成果預覽")
            video_output = gr.Video(label="雙語字幕加黑邊預覽播放器", interactive=False)
            log_output = gr.Textbox(label="Console System Log", placeholder="等待任務觸發...", lines=15, autoscroll=True)

    # 參數打包傳遞
    input_components = [
        video_file, subtitle_file,
        res_select, pad_slider, offset_slider,
        zh_font_dropdown, zh_zh_b := zh_b, zh_zh_i := zh_i, zh_size_input, zh_color_input,
        en_font_dropdown, en_en_b := en_b, en_en_i := en_i, en_size_input, en_color_input
    ]
